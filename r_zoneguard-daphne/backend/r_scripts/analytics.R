...
# Use live DB records and fallback to 0 instead of the hardcoded 148 dummy value
if (file.exists(input_file)) {
  db_data <- tryCatch(fromJSON(input_file), error = function(e) list(total_residents = 0, homeowners = 0, tenants = 0))
} else {
  db_data <- list(total_residents = 0, homeowners = 0, tenants = 0)
}

total <- if (!is.null(db_data$total_residents)) db_data$total_residents else 0
...
# Output using cat() so your Express controller can parse the standard output
